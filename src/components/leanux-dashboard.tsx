...
import { getLeanUxEvents, subscribeLeanUxEvents, type LeanUxEvent } from "@/lib/leanux-recorder"
...
function LeanUxTimeline() {
  const [events, setEvents] = React.useState<LeanUxEvent[]>([])

  React.useEffect(() => {
    const update = () => setEvents(getLeanUxEvents())
    update()
    return subscribeLeanUxEvents(update)
  }, [])

  return (
    <section className="rounded-[24px] bg-white p-6 ring-1 ring-[var(--kita-fog)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="flex items-center gap-2 text-2xl font-semibold"><Activity />Live-Timeline</h2>
          <p className="mt-1 text-sm text-muted-foreground">Startet nach Browser-Refresh bei 0 Sekunden und protokolliert Klicks sowie App-Funktionen.</p>
        </div>
        <div className="rounded-2xl bg-[var(--kita-cloud)] px-4 py-3 text-sm font-bold text-[var(--kita-ink)]">{events.length} Events</div>
      </div>

      <div className="mt-6">
        {events.length ? (
          <ol className="relative flex flex-col gap-4 border-l-2 border-[var(--kita-fog)] pl-6">
            {events.map((event) => (
              <li key={event.id} className="relative">
                <span className="absolute -left-[34px] top-1 flex size-4 rounded-full bg-[var(--kita-blue)] ring-4 ring-white" />
                <div className="rounded-2xl bg-[var(--kita-cloud)] p-4">
                  <div className="flex items-center gap-2 text-sm font-bold text-primary"><Clock3 className="size-4" />{seconds(event.elapsedMs)}</div>
                  <div className="mt-1 text-base font-semibold text-[var(--kita-ink)]">{event.label}</div>
          ...
          <div className="rounded-2xl bg-[var(--kita-cloud)] p-5 text-sm font-medium text-muted-foreground">Noch keine Messung vorhanden. Öffne die App, lade den Browser neu und klicke durch einen Ablauf.</div>
        )}
      </div>
    </section>
  )
...
export function LeanUxDashboard() {
  return (
